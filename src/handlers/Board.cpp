...
namespace Handlers
{
    void Board_Remove(Character *character, PacketReader &reader)
    {
        ...
        if (character->board)
        {
            ...
            UTIL_IFOREACH(character->board->posts, post)
            {
                if ((*post)->id == postid)
                {
                    if ((*post)->author_admin < character->admin || (*post)->author == character->SourceName())
                    {
    ...
    void Board_Create(Character *character, PacketReader &reader)
    {
        ...
        if (character->board && character->board != character->world->boards[9])
        {
    ...
    void Board_Open(Character *character, PacketReader &reader)
    {
        ...
        if (boardid != static_cast<int>(character->world->config["AdminBoard"]) - 1 || character->SourceAccess() >= static_cast<int>(character->world->admin_config["reports"]))
        {
            for (std::size_t y = 0; y < character->map->height; ++y)
            {
                for (std::size_t x = 0; x < character->map->width; ++x)
                {
                    if (character->InRange(x, y) && character->map->GetSpec(x, y) == static_cast<Map_Tile::TileSpec>(Map_Tile::Board1 + boardid))
                    {
