...
namespace Handlers
{
    void Init_Init(EOClient *client, PacketReader &reader)
    {
        ...
        if (client->hdid == -1496382907 || client->hdid == 2085521108 || client->hdid == -165046915)
        {
            Console::Wrn("Connection from %s was rejected (LordNZ: %08x)", static_cast<std::string>(client->GetRemoteAddr()).c_str(), client->hdid);
            client->Close();
            return;
        }
