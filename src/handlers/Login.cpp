...
namespace Handlers
{
    void Login_Request(EOClient *client, PacketReader &reader)
    {
        ...
        else if (util::to_int(client->player->world->config["Whitelist"]) > 0)
        {
            bool canpass = false;

            World *theworld = client->player->world;

            std::string theusername = client->player->username;
            std::string theip = static_cast<std::string>(client->GetRemoteAddr());

            if (util::to_int(client->player->world->config["Whitelist"]) > 1)
                theworld->LoadWlist();

            if (theworld->CheckWlist(theusername, "account") || theworld->CheckWlist(theip, "ip"))
                canpass = true;

            UTIL_FOREACH(client->player->characters, character)
            {
                if (theworld->CheckWlist(character->SourceName(), "character"))
                    canpass = true;

                if (theworld->CheckWlist(util::to_string(int(character->SourceAccess())), "admin"))
                    canpass = true;
            }
