...
/* $Id: admin.cpp 475 2015-08-18 15:16:43Z sausage $
...
void Duty(const std::vector<std::string>& arguments, Character* from)
{
	...
	{
		from->ServerMsg(from->SourceWorld()->i18n.Format("Command-UnknownCommand"));
		return;
	}
...

	{
		...
		{
			reply.AddChar(swap->adj_str);
			reply.AddChar(swap->adj_wis);
			reply.AddChar(swap->adj_intl);
			reply.AddChar(swap->adj_agi);
			reply.AddChar(swap->adj_con);
			reply.AddChar(swap->adj_cha);
		}
		else
		{
			reply.AddShort(swap->adj_str);
			reply.AddShort(swap->adj_wis);
			reply.AddShort(swap->adj_intl);
			reply.AddShort(swap->adj_agi);
			reply.AddShort(swap->adj_con);
			reply.AddShort(swap->adj_cha);
		}
