...
void Arena::Attack(Character *from, Direction direction)
{
	...
	UTIL_FOREACH(this->map->characters, character)
	{
		if (character->arena == this && character->x == target_x && character->y == target_y)
		{
            ...
            int KillItem = util::to_int(util::explode(',', from->world->config["ArenaItem"])[0]);
            int KillAmount = util::to_int(util::explode(',', from->world->config["ArenaItem"])[1]);

            if (KillItem > 0 && KillAmount > 0)
                from->GiveItem(KillItem, KillAmount);

			if (int(from->world->config["ArenaEXP"]) > 0)
                from->GiveEXP(std::min(std::max(int(from->world->config["ArenaEXP"]), 0), int(from->world->config["MaxExp"])));
			...
			if (from->arena->occupants == 2)
			{
                ...
                int WinItem = util::to_int(util::explode(',', from->world->config["ArenaWinsItem"])[0]);
                int WinAmount = util::to_int(util::explode(',', from->world->config["ArenaWinsItem"])[1]);

                if (WinItem > 0 && WinAmount > 0)
                    from->GiveItem(WinItem, WinAmount);

                if (int(from->world->config["ArenaWinsEXP"]) > 0)
                    from->GiveEXP(std::min(std::max(int(from->world->config["ArenaWinsEXP"]), 0), int(from->world->config["MaxExp"])));

			    if (!from->HasAchievement(std::string(this->map->world->achievements_config["ArenaWin.Achievement"])))
                {
                    if (std::string(this->map->world->achievements_config["ArenaWin.Achievement"]) != "0")
                    {
                        from->GiveEXP(std::min(std::max(int(this->map->world->achievements_config["ArenaWin.Reward"]), 0), int(this->map->world->config["MaxExp"])));

                        if (std::string(this->map->world->achievements_config["ArenaWin.Title"]) != "0")
                            from->title = std::string(this->map->world->achievements_config["ArenaWin.Title"]);

                        if (std::string(this->map->world->achievements_config["ArenaWin.Description"]) != "0")
                            from->ServerMsg(std::string(this->map->world->achievements_config["ArenaWin.Description"]));

                        if (std::string(this->map->world->achievements_config["ArenaWin.Announcement"]) == "yes")
                            this->map->world->ServerMsg(util::ucfirst(from->SourceName()) + " earned the achievement '" + std::string(this->map->world->achievements_config["ArenaWin.Achievement"]) + "'.");

                        from->NewAchievement(std::string(this->map->world->achievements_config["ArenaWin.Achievement"]));
                    }
                }
			}
			else
